    ...
    <script>
        ...
        $(function () {
            ...
            $("#sub-btn").click(function () {
                // 编写用于验证的正则
                let unOrPdPattern = /^\w{6,10}$/;
                // 校验用户名
                let usernameVal = $("#username").val();
                if (!unOrPdPattern.test(usernameVal)) {
                    $("span[class='errorMsg']").text("用户名长度需要6-10个字符！");
                    return false;
                }
                // 校验密码
                let passwordVal = $("#password").val();
                if (!unOrPdPattern.test(passwordVal)) {
                    $("span.errorMsg").text("密码长度需要6-10个字符！");
                    return false;
                }
                // 密码二次校验
                ...
                // 校验邮箱
                // 编写用于验证的正则
                let emailPattern = /^[\w-]+@([a-zA-Z]+\.)+[a-zA-Z]+$/
